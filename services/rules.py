...
class RulesEngine:
    @staticmethod
    async def evaluate_transaction(sender_id: str, receiver_id: str, amount: float) -> list[str]:
        triggered_rules = []
        ...
        try:
            ...
            # Use Redis transaction pipeline for atomic execution of operations
            async with r.pipeline(transaction=True) as pipe:
                pipe.zremrangebyscore(redis_key, "-inf", day_ago)
                pipe.zrange(redis_key, 0, -1, withscores=True)
                pipe.zadd(redis_key, {f"{amount}:{now}": now})
                pipe.expire(redis_key, 90000)
                results = await pipe.execute()
            
            # zrange results is at index 1 in pipeline execution results
            ...
            for tx_val, tx_time in recent_txs:
                try:
                    # tx_val is stored as "amount:timestamp" to ensure uniqueness in sorted set
